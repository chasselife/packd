 ...
 * @param color - The hex color value (e.g., '#53b87d')
...
export function getColorClasses(
  color?: string,
  includeButtonClass: boolean = false
): ColorClasses {
  ...
  if (includeButtonClass) {
    ...
    borderClass = colorOption.borderClass.replace(/-700$/, '-500').replace(/^border-primary$/, 'border-primary-500');
    textClass = colorOption.textClass.replace(/-700$/, '-800').replace(/^text-primary$/, 'text-primary-800');
  }
